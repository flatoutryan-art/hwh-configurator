...
function calcJobTotal(units, materials) {
  ...
  units.forEach(u => {
    ...
    total += carcassPrice + carcassBuildUpcharge;
    const hc              = hingeCount(u);
    const finishPerDoor   = PRICING.finish[u.finish || DEFAULT_FINISH] || 0;
    total += finishPerDoor * hc.doors;
    const dc           = drawerCount(u);
    const handlePrice  = PRICING.hardware[u.hardware || DEFAULT_HW] || 0;
    total += handlePrice * (hc.doors + dc);
    const hingePrice   = PRICING.hinge[u.hinge || DEFAULT_HINGE] || 0;
...
export default function App() {
  ...
  async function handleGetQuote() {
    const unitsSummary = units.map((u, i) => {
      ...
      const hc        = hingeCount(u);
      const dc        = drawerCount(u);
      const hingeStr  = hc.total > 0 ? ` | ${hc.total}x ${HINGES.find(h => h.id === (u.hinge || DEFAULT_HINGE))?.label}` : "";
      const runnerStr = dc > 0 ? ` | ${dc} pairs ${RUNNERS.find(r => r.id === (u.runner || DEFAULT_RUNNER))?.label}` : "";
      return `${i+1}. ${ct?.label||u.type} ${u.width}mm — ${getFinish(u.finish).label}${hingeStr}${runnerStr}`;
    }).join("\n");
    try {
      await window.emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
        units_summary:     unitsSummary,
        countertop_type:   COUNTERTOP_TYPES.find(t=>t.id===materials.ctType)?.label || "Not selected",
        countertop_colour: COUNTERTOP_COLOURS.find(c=>c.id===materials.ctColour)?.label || "Not selected",
        carcass_finish:    CARCASS_TYPES_MATERIAL.find(t=>t.id===materials.carcass)?.label || "Not selected",
      });
                ...
                <div style={{ margin:"0 0 14px", padding:"14px 16px", background:`linear-gradient(135deg,${C.orange}22,${C.gold}11)`, border:`1.5px solid ${C.orange}`, borderRadius:8 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <div>
                      ...
                      <div style={{ fontSize:8, color:"#555", marginTop:2 }}>Excl. VAT · labour & installation quoted separately</div>
                    </div>
