...
// ── EmailJS configuration — loaded via CDN script in index.html ───────────────
    ...
    <aside style={{ width:220, background:C.panel, display:"flex", flexDirection:"column", height:"100%", overflow:"hidden" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", minHeight:0 }}>
        ...
        {phase === "layout" && (
          <>
            {[
              { label:"Base Units",          types:["base"]        },
              { label:"Drawer Units",        types:["drawer"]      },
              { label:"Wall Cabinets",       types:["upper"]       },
              { label:"Tall — Single Door",  types:["tall-single"] },
              { label:"Tall — Double Door",  types:["tall-double"] },
              { label:"Corner Units",        types:["corner"]      },
            ].map(group => {
              const items = CARCASS_TYPES.filter(c => group.types.includes(c.type));
              const isFloorGroup = ["base","drawer","tall-single","tall-double","corner"].some(t => group.types.includes(t));
              return (
                <div key={group.label}>
                  <div style={{ padding:"8px 12px 4px", fontSize:7, letterSpacing:2, color:"#444", textTransform:"uppercase", borderBottom:`1px solid ${C.border}22` }}>
                    {group.label}
                    {isFloorGroup && <span style={{ color:C.orange+"66", marginLeft:6 }}>+kick plate</span>}
                  </div>
                  {items.map(c => (
...
export default function App() {
  ...
  // ── handleGetQuote — sends units + materials directly via EmailJS ────────────
  async function handleGetQuote() {
    const unitsSummary = units.map((u, i) => {
      const ct        = CARCASS_TYPES.find(c => c.type === u.type && c.width === u.width);
      const label     = ct?.label || u.type;
      const finish    = getFinish(u.finish).label;
      const hc        = hingeCount(u);
      const dc        = drawerCount(u);
      const hingeStr  = hc.total > 0
        ? ` | ${hc.total}x ${HINGES.find(h => h.id === (u.hinge || DEFAULT_HINGE))?.label}`
        : "";
      const runnerStr = dc > 0
        ? ` | ${dc} pairs ${RUNNERS.find(r => r.id === (u.runner || DEFAULT_RUNNER))?.label}`
        : "";
      return `${i + 1}. ${label} ${u.width}mm — ${finish}${hingeStr}${runnerStr}`;
    }).join("\n");

    const ctType   = COUNTERTOP_TYPES.find(t => t.id === materials.ctType)?.label   || "Not selected";
    const ctColour = COUNTERTOP_COLOURS.find(c => c.id === materials.ctColour)?.label || "Not selected";
    const carcass  = CARCASS_TYPES_MATERIAL.find(t => t.id === materials.carcass)?.label || "Not selected";
    ...
    try {
      await window.emailjs.send(
        ...
        EMAILJS_TEMPLATE_ID,
        {
          ...
          countertop_type:   ctType,
          countertop_colour: ctColour,
          carcass_finish:    carcass,
        }
      );
      alert("Quote sent successfully!");
    } catch (err) {
      console.error("EmailJS handleGetQuote error:", err);
    ...
    <div style={{ height:"100vh", background:C.darkbg, fontFamily:"'Trebuchet MS',sans-serif", color:C.white, display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      <header style={{ padding:`0 ${isPhone?10:20}px`, borderBottom:`3px solid ${C.orange}`, background:C.black, display:"flex", alignItems:"center", gap:isPhone?8:16, height:isPhone?50:58, flexShrink:0 }}>
        ...
        <div style={{ display:"flex", gap:isPhone?6:8, alignItems:"center" }}>
          ...
          {/* Quote */}
          <button onClick={handleGetQuote} style={{ background:C.orange, color:C.white, border:"none", borderRadius:5, padding:isPhone?"7px 10px":"8px 16px", cursor:"pointer", fontSize:isPhone?10:11, fontWeight:"bold", letterSpacing:1, textTransform:"uppercase", fontFamily:"inherit", whiteSpace:"nowrap" }}>
            ...
            {isMobile && (
              <div style={{ display:"flex", justifyContent:"center", marginTop:10, flexShrink:0 }}>
                <button onClick={()=>setPanelOpen(true)} style={{
                  background:C.orange, color:C.white, border:"none", borderRadius:6,
                  padding:"11px 28px", cursor:"pointer", fontSize:12, fontWeight:"bold",
                  letterSpacing:1.5, textTransform:"uppercase", fontFamily:"inherit",
                  boxShadow:"0 4px 16px rgba(217,99,57,0.4)",
                }}>
